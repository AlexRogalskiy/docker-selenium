...
## Docker images for Selenium Standalone Server Hub and Node configurations with Chromium
[Travis CI](https://travis-ci.org/rbonghi/docker-selenium)
...
Chromium
``` bash
$ docker run -d -p 4444:4444 -v /dev/shm:/dev/shm rbonghi/selenium-standalone-chromium:3.141.59-iron
#OR
$ docker run -d -p 4444:4444 --shm-size=2g rbonghi/selenium-standalone-chromium:3.141.59-iron
```
This is a known workaround to avoid the browser crashing inside a docker container, here are the documented issues for
[Chromium](https://code.google.com/p/chromium/issues/detail?id=519952).
...
To inspect visually what the browser is doing use the `standalone-chromium-debug` image. See [Debugging](#debugging) section for details.
...
$ docker run -d -p 4444:4444 --net grid --name selenium-hub rbonghi/selenium-hub:3.141.59-iron
$ docker run -d --net grid -e HUB_HOST=selenium-hub -v /dev/shm:/dev/shm rbonghi/selenium-node-chromium:3.141.59-iron
```
...
services:
  chromium:
    image: rbonghi/selenium-node-chromium:3.141.59-iron
    volumes:
      - /dev/shm:/dev/shm
    depends_on:
      - hub
  ...
  hub:
    image: rbonghi/selenium-hub:3.141.59-iron
    ports:
      - "4444:4444"
```
...
``` bash
$ docker run -d -p 4444:4444 --name selenium-hub rbonghi/selenium-hub:3.141.59-iron
$ docker run -d --link selenium-hub:hub -v /dev/shm:/dev/shm rbonghi/selenium-node-chromium:3.141.59-iron
```
...
``` bash
$ docker run -d -p 4444:4444 -e JAVA_OPTS=-Xmx512m --name selenium-hub rbonghi/selenium-hub:3.141.59-iron
```
...
``` bash
$ docker run -d -p 4444:4444 -e SE_OPTS="-debug" --name selenium-hub rbonghi/selenium-hub:3.141.59-iron
```
...
$ docker run -d -e HUB_HOST=<hub_ip|hub_name> -e HUB_PORT=4444 rbonghi/selenium-node-chromium:3.141.59-iron
```
...
``` bash
# Assuming a hub was already started on the default port
$ docker run -d -p <node_port>:5555 -e HUB_HOST=<hub_ip|hub_name> -e HUB_PORT=4444 -e REMOTE_HOST="http://<node_ip|node_name>:<node_port>" rbonghi/selenium-node-chromium:3.141.59-iron
```
...
``` bash
docker run -d -e SCREEN_WIDTH=1366 -e SCREEN_HEIGHT=768 -e SCREEN_DEPTH=24 rbonghi/selenium-standalone-chromium
```
...
$ docker run -d -e HUB_HOST=<hub_ip|hub_name> -e NODE_MAX_INSTANCES=5 rbonghi/selenium-node-chromium:3.141.59-iron
```
...
``` bash
# Assuming a hub was already started
$ docker run -d -e HUB_HOST=<hub_ip|hub_name> -e NODE_MAX_INSTANCES=5 -e NODE_MAX_SESSION=5 rbonghi/selenium-node-chromium:3.141.59-iron
```
...
``` bash
$ docker run -d --net grid -e HUB_HOST=selenium-hub -e START_XVFB=false -v /dev/shm:/dev/shm rbonghi/selenium-node-chromium
``` 
...
$ docker run -d -p 4444:4444 --net grid --name selenium-hub \
    ...
    rbonghi/selenium-hub:3.141.59-iron
$ docker run -d --net grid -e HUB_HOST=selenium-hub -v /dev/shm:/dev/shm rbonghi/selenium-node-chromium:3.141.59-iron
```
...
``` bash
$ docker run -d -P -p <port4VNC>:5900 --link selenium-hub:hub -v /dev/shm:/dev/shm rbonghi/selenium-node-chromium-debug:3.141.59-iron
```
e.g.:
``` bash
$ docker run -d -P -p 5900:5900 --link selenium-hub:hub -v /dev/shm:/dev/shm rbonghi/selenium-node-chromium-debug:3.141.59-iron
```
to connect to the Chrome node on 5900 and the Firefox node on 5901 (assuming those node are free, and reachable).

And for standalone:
``` bash
$ docker run -d -p 4444:4444 -p <port4VNC>:5900 -v /dev/shm:/dev/shm rbonghi/selenium-standalone-chromium-debug:3.141.59-iron
```
or
``` bash
$ docker run -d -p 4444:4444 -p 5900:5900 -v /dev/shm:/dev/shm rbonghi/selenium-standalone-chromium-debug:3.141.59-iron
```
